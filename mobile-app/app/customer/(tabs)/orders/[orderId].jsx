  ...
  return (
    <View className="flex-1 bg-gray-50 px-4 pt-8">
      ...
      {order.status === 'pending' && (
        <Pressable
          className={`bg-red-500 py-4 rounded-xl mt-6 ${cancelLoading ? 'opacity-60' : ''}`}
          onPress={handleCancel}
          disabled={cancelLoading}
        >
          <Text className="text-white text-center text-lg font-bold">{cancelLoading ? 'Cancelling...' : 'Cancel Order'}</Text>
        </Pressable>
      )}
    </View>
  );
