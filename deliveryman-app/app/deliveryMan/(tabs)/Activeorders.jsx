...
export default function DeliveryActiveOrders() {
  const router = useRouter();
  // We consume the context directly
  const { acceptedOrders, fetchInitialActiveData, loading } = useDeliverySocket();
  const [refreshing, setRefreshing] = React.useState(false);

  const onRefresh = async () => {
    setRefreshing(true);
    await fetchInitialActiveData(); // Reloads active orders from DB
    setRefreshing(false);
  };
  ...
  if (loading) return <Text className="text-center mt-10">Loading Active Orders...</Text>;

  return (
    <View className="flex-1 bg-gray-50">
       <View className="px-4 py-3 bg-white border-b border-gray-200 shadow-sm">
        <Text className="text-lg font-bold text-gray-800">Current Deliveries</Text>
        <Text className="text-xs text-green-600">● Live Updates Enabled</Text>
      </View>
      ...
      >
        {acceptedOrders.length === 0 ? (
          <View className="items-center py-20">
            ...
            <Text className="text-gray-500 mt-4">No active deliveries.</Text>
            <Text className="text-gray-400 text-xs">Accept new requests from the popup.</Text>
          </View>
        ) : (
          acceptedOrders.map((order) => (
            ...
            >
              ...
              <View className="mb-3 p-2 bg-blue-50 rounded-lg">
                ...
                <Text className="text-xs text-blue-700">Name: {order.customer?.name || 'N/A'}</Text>
                <Text className="text-xs text-blue-700">Phone: {order.customer?.phone_number || 'N/A'}</Text>
                <Text className="text-xs text-blue-700">Address: {order?.address || order.customer?.address || 'N/A'}</Text>
              </View>
