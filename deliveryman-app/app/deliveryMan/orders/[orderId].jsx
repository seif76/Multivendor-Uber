...
//   // };
//   // useEffect(() => {
//   //   fetchOrderRef.current = fetchOrderDetails;
//   // });
...
//     // });
//     // socketRef.current.on('deliveryStatusUpdate', ({ orderId: updatedOrderId, status, orderDetails }) => {
//     //   console.log('Delivery status update received:', updatedOrderId, status);
//     //   if (parseInt(updatedOrderId) === parseInt(orderId)) {
//     //     fetchOrderDetails(true);
//     //   }
//     // });
//     socketRef.current.on('deliveryStatusUpdate', ({ orderId: updatedOrderId }) => {
//       console.log('Delivery status update received:', updatedOrderId);
//       if (parseInt(updatedOrderId) === parseInt(orderId)) {
        
//         fetchOrderRef.current?.(true); // ← always calls latest version of fetchOrderDetails
//       }
//     });
...
//   // };
//   const handleStatusUpdate = (newStatus) => {
//     setOrder(prevOrder => prevOrder ? { ...prevOrder, delivery_status: newStatus } : prevOrder);
//     fetchOrderDetails(true);
//   };
...
//   // };
//   const handleCustomerStatusUpdate = (newStatus) => {
//     setOrder(prevOrder => prevOrder ? { ...prevOrder, customer_delivery_status: newStatus } : prevOrder);
//     fetchOrderDetails(true);
//   };
...
export default function DeliverymanOrderDetailsPage() {
  ...
  if (!order) {
    return (
      <View className="flex-1 justify-center items-center bg-gray-50">
        <Text className="text-gray-600 text-lg">Order not found</Text>
        <TouchableOpacity onPress={() => router.back()} className="bg-primary px-6 py-3 rounded-lg mt-4">
          <Text className="text-white font-semibold">Go Back</Text>
        </TouchableOpacity>
      </View>
    );
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="px-4 py-3 bg-white border-b border-gray-200">
        <View className="flex-row items-center justify-between">
          ...
          <View className="flex-row items-center">
            <View className={`w-2 h-2 rounded-full mr-2 ${socketConnected ? 'bg-green-500' : 'bg-red-500'}`} />
            <Text className="text-xs text-gray-600">{socketConnected ? 'Online' : 'Offline'}</Text>
          </View>
        </View>
      </View>
      ...
      >
        ...
        {/* Delivery Confirmation — vendor ↔ deliveryman */}
        <DeliveryConfirmation
          key={`delivery-${order.id}-${order.delivery_status}`}
          order={order}
          onStatusUpdate={() => {}}
        />

        {/* Customer Delivery Confirmation — customer ↔ deliveryman */}
        {order.status === 'shipped' && (
          <CustomerDeliveryConfirmation
            key={`customer-${order.id}-${order.customer_delivery_status}`}
            isCustomer={false}
            order={order}
            onStatusUpdate={() => {}}
          />
